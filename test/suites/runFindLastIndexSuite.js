...
export function runFindLastIndexSuite(findLastIndex) {
  ...
  test("provides current value as first argument to callback", async () => {
    ...
    await findLastIndex(input, value => {
  ...
  test("provides current index as second argument to callback", async () => {
    ...
    await findLastIndex(input, (_, index) => {
  ...
  test("calls callback with an `undefined` `this`-context by default", async () => {
    expect.assertions(1);
    await findLastIndex(of("foo"), testCallback);
  ...
  test("calls callback with the `this`-context provided by `thisArg` argument", async () => {
    expect.assertions(1);
    let expectedThis = {};
    await findLastIndex(of("foo"), testCallback, expectedThis);
  ...
  `("$callbackType callback", async ({ callback }) => {
    test.each`
      ...
      ${"AsyncIterable"} | ${of(true, true, true)}            | ${2}
      ${"Iterable"}      | ${[true, true, true]}              | ${2}
      ${"ArrayLike"}     | ${new ArrayLike(true, true, true)} | ${2}
    ...
    `(
      ...
      async ({ iterableLike, expectedValue }) => {
        expect.assertions(1);
        await expect(
          findLastIndex(iterableLike, callback)
